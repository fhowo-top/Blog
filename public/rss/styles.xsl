      ...
      <body>
        <main class="wrap">
          ...
          <div class="notice">
            ...
            <div>
              <strong>这是一个 RSS 订阅链接</strong>，也称为 Web Feed。
              <p>
                复制地址栏中的 URL 粘贴到你的订阅阅读器中即可订阅，或访问
                <a>
          ...
          <p class="meta">
            ...
            <xsl:choose>
              <xsl:when test="count(/rss/channel/item) = 1"> 篇</xsl:when>
              <xsl:otherwise> 篇</xsl:otherwise>
            </xsl:choose>
            ·
            <a>
              <xsl:attribute name="href"><xsl:value-of select="/rss/channel/link" /></xsl:attribute>
          ...
          <footer>
            由 <a href="https://docs.astro.build/en/recipes/rss/">@astrojs/rss</a> 生成。
            样式表来源 <code>/rss/styles.xsl</code>。
          </footer>
        </main>
      </body>
    </html>
